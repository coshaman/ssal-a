...
def fh5(t, x, z, ell, k, omega):
    ...
    for q in range(1, k + 1):
        for r in range(1, x + ell):
            gvalue = g1(t - 1, r, q, omega)
            hvalue = 0
            for i in range(max(0, r - x), min(r, ell) + 1):
                u = r - i
                if i == 0:
                    tail = fh5(t, x, z, ell, k - q, omega)
                    weight = C(x, r) - C(z, r)
                elif i == ell:
                    tail = gh(t - 1, x + i, z, k - q, omega)
                    weight = C(x, u)
                else:
                    tail = fh5(t, x + i, z, ell - i, k - q, omega)
                    weight = C(x, u)
                hvalue += C(ell, i) * weight * tail
            total += C(k - 1, q - 1) * gvalue * hvalue
    return total
